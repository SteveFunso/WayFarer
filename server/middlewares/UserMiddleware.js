...
export default class UserMiddleware {
  static async validateSignUp(req, res, next) {
    try {
      ...
      if (await UserServices.findUserByEmail(email_address).length !== 0) {
        throw new APIError(400, 'email_address already in use');
      }
  ...
  static async validateSignIn(req, res, next) {
    try {
      ...
      if (await UserServices.findUserByEmail(email_address).length !== 0) {
        console.log('test');
        console.log(await UserServices.findUserByEmail(email_address));
        throw new APIError(400, 'email_address already in use');
      }
      if (typeof is_admin !== 'boolean') {
        throw new APIError(400, 'admin should be boolean');
